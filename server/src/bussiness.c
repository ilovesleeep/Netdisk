...
void rmCmd(Task* task) {
    ...
    // 获取当前目录路径
    MYSQL * mysql = getDBConnection(task->dbpool);      //get之后一定要release
    int pwdid = getPwdId(mysql,task->uid);
    char *curr_path = {0};
    curr_path = getPwd(mysql,pwdid);

    goToRelativeDir(mysql,pwdid,curr_path);
...
int getsCmd(Task* task) {
    ...
    int pwd = getPwdId(mysql, task->uid);
