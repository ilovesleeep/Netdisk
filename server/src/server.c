            ...
            for (int i = 0; i < nready; i++) {
                if (ready_events[i].data.fd == listenfd) {
                    ...
                    requestHandler(ready_events[i].data.fd, pool, user_table,
                                dbpool);
                    // connfd update
                    int slot_idx = hashmapSearch(hashmap, ready_events[i].data.fd); // 若存在返回slot，不存在返回-1
                    slot_idx = hwtUpdate(timer, ready_events[i].data.fd, slot_idx); // 插入的新的slot，也就是上一个curr_idx的上一个
                    hashmapInsert(hashmap, ready_events[i].data.fd, slot_idx); // 更新hashmap
