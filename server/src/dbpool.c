...
static void initDBPoolConfig(DBConnectionPool* dbpool, HashTable* ht) {
    dbpool->host = (const char*)find(ht, "db_host");
    dbpool->user = (const char*)find(ht, "db_user");
    dbpool->passwd = (const char*)find(ht, "db_passwd");
    dbpool->db = (const char*)find(ht, "db_db");
    dbpool->init_size = atoi((const char*)find(ht, "db_init_size"));
    dbpool->max_size = atoi((const char*)find(ht, "db_max_size"));
    dbpool->min_size = atoi((const char*)find(ht, "db_min_size"));
}

DBConnectionPool* initDBPool(HashTable* ht) {
    DBConnectionPool* dbpool =
        (DBConnectionPool*)malloc(sizeof(DBConnectionPool));
    if (!dbpool) {
    ...
    for (int i = 0; i < dbpool->init_size; ++i) {
        // node->pconn
        ...
        pconn = mysql_real_connect(pconn, dbpool->host, dbpool->user,
                                   dbpool->passwd, dbpool->db, 0, NULL, 0);
        if (pconn == NULL) {
